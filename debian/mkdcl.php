...
class debianChangelog
{
    ...
    public function writeChangelog()
    {
        ...
        # Get SVN revision
        $svnrev = isset($ch['keywords']['svnrev']) ? (int) $ch['keywords']['svnrev'] : 1;
        ...
        foreach ($changes as $k => $v) {
            $changelog .= '  * SVN Revision ' . $k . ' - ' . $v['author'] .
            ', on ' . date('r', strtotime($v['date'])) . "\n" .
            '    ' . trim(preg_replace('/\n/ms', "\n    ", $v['msg'])) . "\n\n";
        }

        $res = $ch['package'] . ' (' . $ch['version'] . '-' . $rev . ') ' . $ch['dist'] . '; urgency=' . $ch['keywords']['urgency'] .
        ' ; svnrev=' . $currev .
        "\n\n" .
        rtrim($changelog) . "\n\n" .
